...
export function createGaussianRenderer(canvas: HTMLCanvasElement) {
  ...
  const fsSource = `#version 300 es
    ...
    void main() {
      ...
      // float alpha = exp(-1000.5 * r2);

      mat2 invCov = inverse(v_cov2D + 1e-2 * mat2(1.0));

      // 使用完整的马氏距离计算
      vec2 p = gl_PointCoord * 2.0 - 1.0;
      vec2 invCovP = invCov * p;
      float r2 = dot(p, invCovP);
      float alpha = exp(-1000000.5 * r2);
  ...
  // 构造一个高斯点（用于调试）
  const position = new Float32Array([0, 0, 0]) // 正中间
  const opacity = new Float32Array([0.5]) // 不透明
  const scale = new Float32Array([1, 1, 1]) // 小高斯
  const quat = new Float32Array([0, 0, 0, 1]) // 单位旋转
  const dc = [1.0, 0.0, 0.0] // 红色
  ...
  const p = [0.5, -0.3] // gl_PointCoord * 2.0 - 1.0 得到的 vec2 坐标

  // 添加 1e-6 * 单位矩阵
  const epsilon = 1e-6
  const covWithEps = [
    [v_cov2D[0][0] + epsilon, v_cov2D[0][1]],
    [v_cov2D[1][0], v_cov2D[1][1] + epsilon],
  ]
  ...
  const invCov = inverse2x2(covWithEps)

  // 计算 r2 = dot(p, invCov * p)
  function dot(a: number[], b: number[]): number {
    return a[0] * b[0] + a[1] * b[1]
  }

  function mat2MulVec2(m: number[][], v: number[]): number[] {
    return [m[0][0] * v[0] + m[0][1] * v[1], m[1][0] * v[0] + m[1][1] * v[1]]
  }

  const invCov_p = mat2MulVec2(invCov, p)
  const r2 = dot(p, invCov_p)

  // 输出
  console.log('invCov:', invCov)
  console.log('p:', p)
  console.log('invCov * p:', invCov_p)
  console.log('r2:', r2)
}
